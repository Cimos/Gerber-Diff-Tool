...
run anywhere. Layers are located by file path rather than pairing key so the
test is independent of which pairing strategy ran.
"""
...
from gerberdiff.models import LayerPair  # noqa: E402
...
def _find(substring: str) -> LayerPair:
    """Locate a fixture pair by a substring of either side's file name."""
    pairs = pair_layers(FIXTURES / "revA", FIXTURES / "revB")
    for pair in pairs:
        for path in (pair.path_a, pair.path_b):
            if path is not None and substring.lower() in path.name.lower():
                return pair
    raise AssertionError(f"no pair matching {substring!r} in {[p.key for p in pairs]}")
